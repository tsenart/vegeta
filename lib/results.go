...
func (r Result) Equal(other Result) bool {
	return r.Attack == other.Attack &&
		...
		r.ResponseBodyLength == other.ResponseBodyLength &&
		r.Error == other.Error &&
...
// HTTP status code, request latency in ns, bytes out, bytes in, response body length,
...
func NewCSVEncoder(w io.Writer) Encoder {
	...
	return func(r *Result) error {
		err := enc.Write([]string{
			...
			strconv.FormatInt(r.ResponseBodyLength, 10),
			r.Error,
...
func NewCSVDecoder(rd io.Reader) Decoder {
	...
	return func(r *Result) error {
		...
		if r.ResponseBodyLength, err = strconv.ParseInt(rec[5], 10, 64); err != nil {
			return err
		}

		r.Error = rec[6]
		if r.Body, err = base64.StdEncoding.DecodeString(rec[7]); err != nil {
			return err
		}
