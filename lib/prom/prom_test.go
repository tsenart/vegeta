...
func TestMetrics_Observe_FailCounterIncremented(t *testing.T) {
	...
	// Observe a result with an error.
	...
	// Observe a second error result with the same labels.
	pm.Observe(&vegeta.Result{
		URL:     "http://example.com/fail",
		Method:  "GET",
		Code:    500,
	...
	// Observe a success result (no error) - should not increment fail counter.
	pm.Observe(&vegeta.Result{
	...
	// Collect and verify the fail counter value.
	var m dto.Metric
