...
func attackCmd() command {
	...
	fs.StringVar(&opts.server, "server", "", "TLS server name")
...
func tlsConfig(opts *attackOpts) (c *tls.Config, err error) {
	c = &tls.Config{ServerName: opts.server}

	files := map[string][]byte{}
	filenames := append([]string{opts.certf, opts.keyf}, opts.rootCerts...)
	...
	if cert, ok := files[opts.certf]; ok {
		key, ok := files[opts.keyf]
		if !ok {
			key = cert
		}
